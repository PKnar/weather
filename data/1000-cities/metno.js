//create 1000 cities with lng lat and alti file
//query metno for all of them and put the data in weather-metno table
const fs = require("fs");
const convert = require("xml-js");
const request = require("sync-request");
const sleep = require("system-sleep");

const cities = fs.readFileSync("1000-cities.json", "utf-8");
...
for (const location of cityObj) {
  try {
    const response = request(
      ...
      `https://api.met.no/weatherapi/locationforecast/1.9/?lat=${
        location.lat
      }&lon=${location.lng}&msl=${location.alt}`
    );
    sleep(2000);
    ...
    const metNo = {
      ...
      weather: [
        {
          sourceApi: `https://api.met.no/weatherapi/locationforecast/1.9/?lat=${
            location.lat
          }&lon=${location.lng}&msl=${location.alt}`,
          geohash3,
          geohash5,
  ...
  fs.writeFileSync(
    "metNoWeatherOutput.json",
    JSON.stringify(metNoFinalArray, null, 2)
  );
  fs.writeFileSync(
    `./weatherOutput/WeatherOutput${timestamp}.json`,
    JSON.stringify(metNoFinalArray, null, 2)
  );
}
